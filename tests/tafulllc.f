...
C
      IF( FIRST ) THEN
         FIRST = .FALSE.
         OPEN(UNIT= IWRT,FILE= 'tafulllc-das.dat',STATUS= 'UNKNOWN')
         OPEN(UNIT= NUMOUT,FILE= 'tafulllc-das-n.dat',STATUS= 'UNKNOWN')
         RTOL  = 1.0D-7
         ATOL  = 1.7D-9
         WRITE(*,'('//''' '''//',A'//'   '//')')
...
C ...
      ...
      GG(2) = (Y(1)-UET)/R0 + Y(2)/R1+(Y(2)-UB)/R2+(1.0D0-ALPHA)*FAC1
      GG(3) = C2*YP(3)         + Y(3)/R3-FAC1
      GG(4) = C3*(YP(4)-YP(5)) + (Y(4)-UB)/R4+ALPHA*FAC1
      GG(5) = (Y(4)-UB)/R4+ALPHA*FAC1 +
     &        Y(5)/R5+(Y(5)-UB)/R6+(1.0D0-ALPHA)*FAC2
      GG(6) = C4*YP(6)         + Y(6)/R7-FAC2
      GG(7) = Y(8)/R9 + (Y(7)-UB)/R8+ALPHA*FAC2
...
C ...
   ...
   20 CONTINUE
C ...
      DGG(1,1) = C1*CJ + 1.0D0/R0
      DGG(1,2) =-C1*CJ
C ...
      DGG(2,1) = 1.0D0/R0
      DGG(2,2) = 1.0D0/R1+1.0D0/R2+(1.0D0-ALPHA)*FAC1P
      DGG(2,3) =-(1.0D0-ALPHA)*FAC1P
C ...
      DGG(3,2) =-FAC1P
      DGG(3,3) = C2*CJ + 1.0D0/R3+FAC1P
C ...
      DGG(4,2) = ALPHA*FAC1P
      DGG(4,3) =-ALPHA*FAC1P
      DGG(4,4) = C3*CJ + 1.0D0/R4
      DGG(4,5) =-C3*CJ
C ...
      DGG(5,2) = ALPHA*FAC1P
      DGG(5,3) =-ALPHA*FAC1P
      DGG(5,4) = 1.0D0/R4
      DGG(5,5) = 1.0D0/R5+1.0D0/R6+(1.0D0-ALPHA)*FAC2P
      DGG(5,6) =-(1.0D0-ALPHA)*FAC2P
C ...
      DGG(6,5) = 1.0D0/R7-FAC2P
      DGG(6,6) = C4*CJ + FAC2P
C ...
      DGG(7,5) = ALPHA*FAC2P
      DGG(7,6) =-ALPHA*FAC2P
...
C
      IF( TASK.EQ.'START' ) THEN
         ...
         WRITE(IWRT,5)
 5       FORMAT(/
     &   'TransAmp (LC w/ Full Jacobian) example problem for DASSL')
         WRITE(LUN,10) IOUT(NST),T
         WRITE(LUN,40) (Y(I),I= 1,NEQ,1)
      ENDIF
C
