...
C
      IF( FIRST ) THEN
         FIRST = .FALSE.
         OPEN(UNIT= IWRT,FILE= 'tafull-das.dat',STATUS= 'UNKNOWN')
         OPEN(UNIT= NUMOUT,FILE= 'tafull-das-n.dat',STATUS= 'UNKNOWN')
         RTOL  = 1.0D-7
         ATOL  = 1.7D-9
         WRITE(*,'('//''' '''//',A'//'   '//')')
...
C ...
      DGG(2,1) =-C1*CJ
      DGG(2,2) = C1*CJ + 1.0D0/R1+1.0D0/R2+(1.0D0-ALPHA)*FAC1P
...
C ...
      DGG(5,4) =-C3*CJ
      DGG(5,5) = C3*CJ + 1.0D0/R5+1.0D0/R6+(1.0D0-ALPHA)*FAC2P
...
C
      IF( TASK.EQ.'START' ) THEN
         ...
         WRITE(LUN,5)
         WRITE(IWRT,5)
 5       FORMAT(/'TransAmp (Full Jacobian) example problem for DASSL')
         WRITE(LUN,10) IOUT(NST),T
         WRITE(LUN,40) (Y(I),I= 1,NEQ,1)
      ENDIF
C
